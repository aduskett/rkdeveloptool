...
#define tt2 (2 * tt)
unsigned int s[tt2 + 1]; // Syndrome values

unsigned int rr; // redundant length		// BCH code parameters

unsigned int p[mm + 1];
unsigned int alpha_to[nn + 1], index_of[nn + 1]; // Galois field
unsigned int gg[rr_max + 1];                     // Generator polynomial
...
// get crc32 value
unsigned int CRC_32(unsigned char *pData, const unsigned int ulSize) {
  unsigned int i;
...
unsigned int crc32_le(unsigned int crc, unsigned char *p, unsigned int len) {
  auto b = reinterpret_cast<unsigned int *>(p);
  const unsigned int *tab = crc32table_le;
  crc = crc ^ 0xFFFFFFFF;
  if (reinterpret_cast<long>(b) & 3 && len) {
    do {
      auto *p_char = reinterpret_cast<unsigned char *>(b);
      DO_CRC(*p_char++);
      b = reinterpret_cast<unsigned int *>(p_char);
    } while (--len && reinterpret_cast<long>(b) & 3);
  }
  if (len >= 4) {
    const unsigned int save_len = len & 3;
    len = len >> 2;
    --b;
    do {
      crc ^= *++b;
  ...
  if (len) {
    do {
      auto *p_uchar = reinterpret_cast<unsigned char *>(b);
      DO_CRC(*p_uchar++);
      b = reinterpret_cast<unsigned int *>(p_uchar);
    } while (--len);
  }
  crc = crc ^ 0xFFFFFFFF;
  return crc;
...
void CRCBuildTable16(const unsigned short aPoly, unsigned short *crcTable) {
  unsigned short i, j;
  unsigned short nData;
  unsigned short nAccum;

  for (i = 0; i < 256; i++) {
    nData = static_cast<unsigned short>(i << 8);
    nAccum = 0;
    for (j = 0; j < 8; j++) {
...
unsigned short CRC_16(const unsigned char *aData, const unsigned int aSize) {
  unsigned int i;
...
void P_RC4(unsigned char *buf, const unsigned short len) {
  unsigned char S[256], K[256], temp;
  unsigned short i, j, t, x;
  const unsigned char key[16] = {124, 78, 3,   4,  85, 5,  9,  7,
...
void bch_encode(const unsigned char *encode_in, unsigned char *encode_out) {
  unsigned int i, j;
  bool feed_back;
  unsigned int bch1 = 0;
  unsigned int bch2 = 0;
...
unsigned short CRC_Calculate(unsigned short crc, const unsigned char ch) {
  unsigned int i;
  for (i = 0x80; i != 0; i >>= 1) {
    if ((crc & 0x8000) != 0) {
      crc <<= 1;
...
unsigned short CRC_CCITT(const unsigned char *p, unsigned int CalculateNumber) {
...
void gen_poly() {
  unsigned int gen_roots[nn + 1],
      gen_roots_true[nn + 1]; // Roots of generator polynomial
  ...
  for (i = 2; i <= rr; i++) {
    ...
    for (j = i - 1; j > 0; j--)
      if (gg[j] != 0) {
        gg[j] =
            gg[j - 1] ^
            alpha_to[(index_of[gg[j]] + index_of[alpha_to[gen_roots[i]]]) % nn];
      } else {
        gg[j] = gg[j - 1];
      }
  ...
  ggx1 = gg[103] | gg[102] << 1 | gg[101] << 2 | gg[100] << 3 | gg[99] << 4 |
         gg[98] << 5 | gg[97] << 6 | gg[96] << 7 | gg[95] << 8 | gg[94] << 9 |
         gg[93] << 10 | gg[92] << 11 | gg[91] << 12 | gg[90] << 13 |
         gg[89] << 14 | gg[88] << 15 | gg[87] << 16 | gg[86] << 17 |
         gg[85] << 18 | gg[84] << 19 | gg[83] << 20 | gg[82] << 21 |
         gg[81] << 22 | gg[80] << 23 | gg[79] << 24 | gg[78] << 25 |
         gg[77] << 26 | gg[76] << 27 | gg[75] << 28 | gg[74] << 29 |
         gg[73] << 30 | gg[72] << 31;
  ggx2 = gg[71] | gg[70] << 1 | gg[69] << 2 | gg[68] << 3 | gg[67] << 4 |
         gg[66] << 5 | gg[65] << 6 | gg[64] << 7 | gg[63] << 8 | gg[62] << 9 |
         gg[61] << 10 | gg[60] << 11 | gg[59] << 12 | gg[58] << 13 |
         gg[57] << 14 | gg[56] << 15 | gg[55] << 16 | gg[54] << 17 |
         gg[53] << 18 | gg[52] << 19 | gg[51] << 20 | gg[50] << 21 |
         gg[49] << 22 | gg[48] << 23 | gg[47] << 24 | gg[46] << 25 |
         gg[45] << 26 | gg[44] << 27 | gg[43] << 28 | gg[42] << 29 |
         gg[41] << 30 | gg[40] << 31;
  ggx3 = gg[39] | gg[38] << 1 | gg[37] << 2 | gg[36] << 3 | gg[35] << 4 |
         gg[34] << 5 | gg[33] << 6 | gg[32] << 7 | gg[31] << 8 | gg[30] << 9 |
         gg[29] << 10 | gg[28] << 11 | gg[27] << 12 | gg[26] << 13 |
         gg[25] << 14 | gg[24] << 15 | gg[23] << 16 | gg[22] << 17 |
         gg[21] << 18 | gg[20] << 19 | gg[19] << 20 | gg[18] << 21 |
         gg[17] << 22 | gg[16] << 23 | gg[15] << 24 | gg[14] << 25 |
         gg[13] << 26 | gg[12] << 27 | gg[11] << 28 | gg[10] << 29 |
         gg[9] << 30 | gg[8] << 31;
  ggx4 = gg[7] | gg[6] << 1 | gg[5] << 2 | gg[4] << 3 | gg[3] << 4 |
         gg[2] << 5 | gg[1] << 6;
}

void generate_gf() {
  unsigned int i;
  unsigned int mask; // Register states
