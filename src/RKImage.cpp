...
unsigned int CRKImage::GetVersion() { return m_version; }
unsigned int CRKImage::GetMergeVersion() { return m_mergeVersion; }
STRUCT_RKTIME CRKImage::GetReleaseTime() { return m_releaseTime; }
ENUM_RKDEVICE_TYPE CRKImage::GetSupportDevice() { return m_supportDevice; }
ENUM_OS_TYPE CRKImage::GetOsType() {
  unsigned int *pOsType = reinterpret_cast<unsigned int *>(&m_reserved[4]);
  return static_cast<ENUM_OS_TYPE>(*pOsType);
}

unsigned short CRKImage::GetBackupSize() {
  const auto *pBackupSize = reinterpret_cast<unsigned short *>(&m_reserved[12]);
  return *pBackupSize;
}
unsigned int CRKImage::GetBootOffset() { return m_bootOffset; }
unsigned int CRKImage::GetBootSize() { return m_bootSize; }
unsigned int CRKImage::GetFWOffset() { return m_fwOffset; }
long long CRKImage::GetFWSize() { return m_fwSize; }
bool CRKImage::SaveBootFile(const std::string &filename) {
  ...
  do {
    constexpr unsigned int dwBufferSize = 1024;
    BYTE buffer[1024];
    const unsigned int dwReadSize = dwBootSize >= 1024 ? dwBufferSize : dwBootSize;
    const size_t iRead = fread(buffer, 1, dwReadSize, m_pFile);
    if (iRead != static_cast<int>(dwReadSize)) {
      fclose(file);
      return false;
    }
...
bool CRKImage::SaveFWFile(const std::string &filename) {
  FILE *file = fopen(filename.c_str(), "wb+");
  if (!file) {
    return false;
  }
  ...
  do {
    constexpr unsigned int dwBufferSize = 1024;
    BYTE buffer[1024];
    const unsigned int dwReadSize = dwFWSize >= 1024 ? dwBufferSize : dwFWSize;
    const size_t iRead = fread(buffer, 1, dwReadSize, m_pFile);
    if (iRead != static_cast<int>(dwReadSize)) {
      fclose(file);
      return false;
...
bool CRKImage::GetData(const long long dwOffset, const unsigned int dwSize,
                       PBYTE lpBuffer) {
  ...
  const unsigned int uiActualRead = fread(lpBuffer, 1, dwSize, m_pFile);
  if (dwSize != uiActualRead) {
    return false;
  }
  return true;
}
void CRKImage::GetReservedData(PBYTE &lpData, unsigned short &usSize) {
  lpData = m_reserved;
  usSize = IMAGE_RESERVED_SIZE;
}

CRKImage::CRKImage(std::string filename, bool &bCheck) {
  ...
  if (!bOnlyBootFile) {
    long long ulFwSize;
    long long nMd5DataSize;
    fseeko(m_pFile, 0, SEEK_SET);
    iRead = fread(&imageHead, 1, sizeof(STRUCT_RKIMAGE_HEAD), m_pFile);
    ...
    if (imageHead.reserved[14] == 'H' && imageHead.reserved[15] == 'I') {
      ulFwSize = *reinterpret_cast<unsigned int *>(&imageHead.reserved[16]);
      ulFwSize <<= 32;
  ...
  if (iRead != static_cast<int>(m_bootSize)) {
    bCheck = false;
    return;
  }
  bool bRet;
  ...
  if (bOnlyBootFile) {
    ...
    pOsType = (unsigned int *)&m_reserved[4];
    *pOsType = static_cast<unsigned int>(RK_OS);
    fclose(m_pFile);
    m_pFile = nullptr;
  }
  bCheck = true;
...
long long CRKImage::GetImageSize() { return m_fileSize; }
long long CRKImage::GetMd5Data(PBYTE &lpMd5, PBYTE &lpSignMd5) {
  lpMd5 = m_md5;
  lpSignMd5 = m_signMd5;
  return m_signMd5Size;
}
bool CRKImage::GetSignFlag() { return m_bSignFlag; }