...
#define BUS_ID(id) ((id & 0x0000FF00) >> 8)

int CRKScan::GetDEVICE_COUNTS() { return m_list.size(); }

unsigned int CRKScan::GetMSC_TIMEOUT() { return m_waitMscSecond; }

unsigned int CRKScan::GetRKUSB_TIMEOUT() { return m_waitRKusbSecond; }

void CRKScan::SetMSC_TIMEOUT(const unsigned int value) {
  m_waitMscSecond = value;
}

void CRKScan::SetRKUSB_TIMEOUT(const unsigned int value) {
  m_waitRKusbSecond = value;
}

CRKScan::CRKScan(const unsigned int uiMscTimeout,
                 const unsigned int uiRKusbTimeout) {
...
bool CRKScan::FindRockusbVidPid(ENUM_RKDEVICE_TYPE type, unsigned short &usVid,
                                unsigned short &usPid) {
  bool bRet = false;
  for (const auto &i : m_deviceConfigSet) {
    if (i.emDeviceType == type) {
      usVid = i.usVid;
      usPid = i.usPid;
      bRet = true;
      break;
...
void CRKScan::AddRockusbVidPid(const unsigned short newVid,
                               const unsigned short newPid,
                               const unsigned short oldVid,
                               const unsigned short oldPid) {
  ...
  for (i = 0; i < m_deviceConfigSet.size(); i++) {
    if (m_deviceConfigSet[i].usVid == oldVid &&
        m_deviceConfigSet[i].usPid == oldPid) {
      config.usVid = newVid;
      config.usPid = newPid;
...
void CRKScan::SetVidPid(unsigned short mscVid, unsigned short mscPid) {
  STRUCT_DEVICE_CONFIG config;
  m_deviceConfigSet.clear();
...
int CRKScan::FindWaitSetPos(const RKDEVICE_CONFIG_SET &waitDeviceSet,
                            const unsigned short vid,
                            const unsigned short pid) {
  int pos = -1;
  for (int i = 0; i < waitDeviceSet.size(); i++) {
...
int CRKScan::FindConfigSetPos(const RKDEVICE_CONFIG_SET &devConfigSet,
                              const unsigned short vid,
                              const unsigned short pid) {
  int pos = -1;
  for (int i = 0; i < devConfigSet.size(); i++) {
...
void CRKScan::EnumerateUsbDevice(RKDEVICE_DESC_SET &list,
                                 unsigned int &uiTotalMatchDevices) {
  STRUCT_RKDEVICE_DESC desc;
  libusb_device_descriptor descriptor{};
  uiTotalMatchDevices = 0;
  libusb_device **pDevs = nullptr;
  auto ret = libusb_get_device_list(nullptr, &pDevs);
  if (ret < 0) {
    if (m_log) {
      m_log->Record("Error:EnumerateUsbDevice-->get_device_list failed,err=%d!",
                    ret);
    }
    return;
  }
  const ssize_t cnt = ret;
  for (int i = 0; i < cnt; i++) {
    libusb_device *dev = pDevs[i];
    if (dev) {
      ...
      if (ret < 0) {
        ...
        if (m_log) {
          m_log->Record(
              "Error:EnumerateUsbDevice-->get_device_descriptor failed,err=%d!",
              ret);
        }
        return;
      }
      ...
      desc.pUsbHandle = static_cast<void *>(dev);
...
void CRKScan::FreeDeviceList(RKDEVICE_DESC_SET &list) {
  for (auto &[usVid, usPid, usbcdUsb, uiLocationID, emUsbType, emDeviceType,
              pUsbHandle] : list) {
    if (pUsbHandle) {
      libusb_unref_device(static_cast<libusb_device *>(pUsbHandle));
      pUsbHandle = nullptr;
    }
  }
  list.clear();
...
bool CRKScan::IsRockusbDevice(ENUM_RKDEVICE_TYPE &type,
                              const unsigned short vid,
                              const unsigned short pid) {
...
int CRKScan::Search(const unsigned int type) {
  ...
  for (iter = m_list.begin(); iter != m_list.end();) {
    ...
    if (iter->pUsbHandle) {
      libusb_unref_device(static_cast<libusb_device *>(iter->pUsbHandle));
      iter->pUsbHandle = nullptr;
    }
  ...
  for (iter = m_list.begin(); iter != m_list.end(); iter++) {
    if (FindConfigSetPos(m_deviceMscConfigSet, iter->usVid, iter->usPid) !=
      ...
      unsigned short usTemp = iter->usbcdUsb;
      usTemp = usTemp & 0x1;
      if (usTemp == 0)
...
bool CRKScan::MutexWait(UINT_VECTOR &vecExistedDevice,
                        STRUCT_RKDEVICE_DESC &device, ENUM_RKUSB_TYPE usbType,
                        unsigned short usVid, unsigned short usPid) {
  device_list_iter iter;
  unsigned int uiWaitSecond;
  unsigned int iFoundCount = 0;
  bool bFound = false;
  ...
  while (difftime(time(&tmNow), tmInit) <= uiWaitSecond) {
    const unsigned int iRet = Search(RKUSB_MASKROM | RKUSB_LOADER | RKUSB_MSC);
    if (iRet == vecExistedDevice.size() + 1) {
      for (const unsigned int i : vecExistedDevice) {
        for (iter = m_list.begin(); iter != m_list.end();) {
          if (iter->uiLocationID == i) {
            iter = m_list.erase(iter);
          } else
            iter++;
        }
...
bool CRKScan::MutexWaitPrepare(UINT_VECTOR &vecExistedDevice,
                               const unsigned int uiOfflineDevice) {
  int iRet2;
  ...
  for (auto &[usVid, usPid, usbcdUsb, uiLocationID, emUsbType, emDeviceType,
              pUsbHandle] : m_list) {
    if (uiLocationID != uiOfflineDevice) {
      vecExistedDevice.push_back(uiLocationID);
    } else {
      bFound = true;
    }
  }
...
bool CRKScan::Wait(STRUCT_RKDEVICE_DESC &device, ENUM_RKUSB_TYPE usbType,
                   unsigned short usVid, unsigned short usPid) {
  RKDEVICE_DESC_SET deviceList;
  ENUM_RKUSB_TYPE curDeviceType;
  ENUM_RKDEVICE_TYPE devType;
  device_list_iter iter;
  unsigned int totalDevice;
  unsigned int uiWaitSecond;
  unsigned int iFoundCount = 0;
  bool bRet = false;
  if (usbType == RKUSB_MSC)
  ...
  while (difftime(time(&tmNow), tmInit) <= uiWaitSecond) {
    ...
    for (iter = deviceList.begin(); iter != deviceList.end(); iter++) {
      if (BUS_ID(iter->uiLocationID) != BUS_ID(device.uiLocationID) ||
          (BUS_ID(iter->uiLocationID) == BUS_ID(device.uiLocationID) &&
           iter->uiLocationID >= device.uiLocationID)) {
        if (usVid != 0 || usPid != 0) {
        ...
        } else if (FindConfigSetPos(m_deviceMscConfigSet, iter->usVid,
                                    iter->usPid) != -1) {
    ...
    if (iFoundCount >= 8) {
      ...
      libusb_ref_device(static_cast<libusb_device *>(device.pUsbHandle));
...
int CRKScan::GetPos(unsigned int locationID) {
  int pos = 0;
  bool bFound = false;
  for (const auto &[usVid, usPid, usbcdUsb, uiLocationID, emUsbType,
                    emDeviceType, pUsbHandle] : m_list) {
    if (locationID == uiLocationID) {
      bFound = true;
      break;
    }
    pos++;
